    ...
    public class VesselSpawnerWindow : MonoBehaviour
    {
        ...
        private void WindowVesselSpawner(int id)
        {
            ...
            if (BDArmorySettings.SHOW_TOURNAMENT_OPTIONS)
            {
                ...
                GUI.Label(SLeftSliderRect(++line), $"{Localizer.Format("#LOC_BDArmory_Settings_TournamentTimeWarpBetweenRounds")}: ({BDArmorySettings.TOURNAMENT_TIMEWARP_BETWEEN_ROUNDS}min)", leftLabel); // TimeWarp Between Rounds
                BDArmorySettings.TOURNAMENT_TIMEWARP_BETWEEN_ROUNDS = Mathf.RoundToInt(GUI.HorizontalSlider(SRightSliderRect(line), BDArmorySettings.TOURNAMENT_TIMEWARP_BETWEEN_ROUNDS / 5f, 0f, 60f)) * 5;
