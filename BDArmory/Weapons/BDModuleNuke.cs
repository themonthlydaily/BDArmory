...
namespace BDArmory.Weapons
{
    class BDModuleNuke : PartModule
    {
                ...
                if (BDACompetitionMode.Instance.competitionIsActive) //only begin checking engine state after comp start
                {
                    if (engineCore)
                    {
                        ...
                        if (fuelleft <= 0)
                        {
                            ...
                            {
                                if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.RWPS3R2NukeModule]: nerva on " + Sourcevessel + " is out of fuel.");
                        ...
                        if (engine != null)
                        {
                            if (!engine.isEnabled || !engine.EngineIgnited) //so this is getting tripped by multimode engines toggling from wet/dry
                            {
                                if (!hasDetonated)
                                {
                                    if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.RWPS3R2NukeModule]: nerva on " + Sourcevessel + " is Off, detonating");
                            ...
                            if (engine.thrustPercentage < 100)
                            {
                                if (part.Modules.GetModule<HitpointTracker>().Hitpoints == part.Modules.GetModule<HitpointTracker>().GetMaxHitpoints())
                                {
                                    if (!hasDetonated)
                                    {
                                        if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.RWPS3R2NukeModule]: nerva on " + Sourcevessel + " is manually thrust limited, detonating");
                                        Detonate(); //nuke engine off after comp start, detonate.
                                    }
                                }
                            }
        ...
        void CheckAttached(Vessel v)
        {
            ...
            {
                if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.RWPS3R2NukeModule]: Nuclear engine on " + Sourcevessel + " has become detached.");
                goingCritical = true;
        ...
        {
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.RWPS3R2NukeModule]: Nuclear engine on " + Sourcevessel + " going critical in " + delay.ToString("0.0") + "s.");
            goingCritical = true;
        ...
        public void Detonate()
        {
            ...
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.BDModuleNuke]: Running Detonate() on nukeModule in vessel " + Sourcevessel);
