    ...
    public class CircularSpawning : VesselSpawnerBase
    {
                ...
                else
                {
                    ...
                    else
                    {
                        foreach (var vessel in spawnedVessels.Select(v => v.Value))
                        {
                            ...
                            {
                                if (BDArmorySettings.DEBUG_OTHER) Debug.Log("[BDArmory.CircularSpawning]: " + vessel.vesselName + " didn't activate engines on AG10! Activating ALL their engines.");
        ...
        IEnumerator SpawnSingleVessel(VesselSpawnConfig vesselSpawnConfig)
        {
            ...
            if (terrainAltitude > 0 && Physics.Raycast(ray, out hit, distanceToCoMainBody, (int)LayerMasks.Scenery))
            {
                distance = hit.distance;
                localSurfaceNormal = hit.normal;
                if (BDArmorySettings.DEBUG_OTHER) Debug.Log("[BDArmory.CircularSpawning]: found terrain for spawn adjustments");
            }
            else
            {
                ...
                localSurfaceNormal = radialUnitVector;
                if (BDArmorySettings.DEBUG_OTHER) Debug.Log("[BDArmory.CircularSpawning]: failed to find terrain for spawn adjustments");
            }
            // Rotation
