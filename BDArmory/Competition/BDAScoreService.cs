    ...
    public class BDAScoreService : MonoBehaviour
    {
        ...
        private List<RecordModel> BuildRecords(string hash, HeatModel heat)
        {
            ...
            foreach (string playerName in playerNames)
            {
                ...
                record.rocket_strikes_out = ComputeTotalRocketStrikesOut(playerName);
                record.rocket_strikes_in = ComputeTotalRocketStrikesIn(playerName);
                record.rocket_parts_out = ComputeTotalRocketPartsOut(playerName);
                record.rocket_parts_in = ComputeTotalRocketPartsIn(playerName);
                record.rocket_dmg_out = ComputeTotalRocketDamageOut(playerName);
                record.rocket_dmg_in = ComputeTotalRocketDamageIn(playerName);
