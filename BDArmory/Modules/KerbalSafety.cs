...
namespace BDArmory.Modules
{
    ...
    public class KerbalSafetyManager : MonoBehaviour
    {
        ...
        {
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDArmory.KerbalSafety]: Manually setting position of " + kerbal.vessel.vesselName + " for " + realTime + "s of real-time.");
            ...
            {
                if (BDArmorySettings.DEBUG_LABELS) verticalSpeedAdjustment = 3f * (float)gee.magnitude - verticalSpeed;
            ...
            else
            {
                ...
                if (BDArmorySettings.DEBUG_LABELS) verticalSpeedAdjustment = 1.5f * (float)gee.magnitude;
            }
            ...
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log($"[BDArmory.KerbalSafety]: Setting {kerbal.vessel.vesselName}'s position to {position:F2} ({kerbal.vessel.GetWorldPos3D():F2}, altitude: {kerbal.vessel.radarAltitude:F2}, {kerbal.vessel.altitude:F2}) and velocity to {velocity.magnitude:F2} ({verticalSpeed:F2}m/s vertically, adjusted by {verticalSpeedAdjustment:F2}m/s)");
            ...
            while (kerbal != null && kerbal.isActiveAndEnabled && kerbal.vessel != null && kerbal.vessel.isActiveAndEnabled && Time.realtimeSinceStartup - startTime < realTime)
            {
                ...
                if (verticalSpeed < 0f && kerbal.vessel.radarAltitude + verticalSpeed * (realTime - (Time.realtimeSinceStartup - startTime)) < 100f)
                {
                    ...
                    if (BDArmorySettings.DEBUG_LABELS) verticalSpeedAdjustment = Vector3.Dot(-gee.normalized, gee * verticalSpeed / 10f * Time.fixedDeltaTime);
                }
                else
                {
                    velocity = velocity * 0.968f + gee * Time.fixedDeltaTime;
                    if (BDArmorySettings.DEBUG_LABELS) verticalSpeedAdjustment = Vector3.Dot(-gee.normalized, gee * Time.fixedDeltaTime);
                }
                verticalSpeed = Vector3.Dot(-gee.normalized, velocity);
    ...
    public class KerbalSafety : MonoBehaviour
    {
        ...
        {
            StopAllCoroutines();
            if (KerbalSafetyManager.Instance.safetyLevel != KerbalSafetyLevel.Off && !recovered && BDArmorySettings.DEBUG_LABELS)
            {
        ...
        {
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log(arg1);
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log(arg2);
        }
