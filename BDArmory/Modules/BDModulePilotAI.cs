    ...
    public class BDModulePilotAI : BDGenericAIBase, IBDAIControl
    {
        ...
        protected override void Start()
        {
            ...
            if (HighLogic.LoadedSceneIsFlight)
            {
                ...
                GameEvents.onCollision.Add(Collision);
            }
            SetSliderClamps("turnRadiusTwiddleFactorMin", "turnRadiusTwiddleFactorMax");
            SetSliderClamps("DynamicDampingMin", "DynamicDampingMax");
        }

        private void Collision(EventReport data)
        {
            var v = data.origin.vessel;
            {
                if (v == vessel)
                {
                    ScoringData vData;
                    vData = BdComp.Scores[vessel.GetName()];

                    if (!vData.hasReachedVessel)
                        vData.hasReachedVessel = true;
                    if (!vData.otherVesselScoringData.hasReachedVessel)
                        vData.otherVesselScoringData.hasReachedVessel = true;

                    Debug.Log("DEBUG " + vessel.vesselName + " has reached vessel " + vData.hasReachedVessel);
                    collisionDetected = true;
                    collisionTime = Planetarium.GetUniversalTime();
                }
            }
        }
        ...
        void FixedUpdate()
        {
            ...
            //ramming log
            if (HighLogic.LoadedSceneIsFlight && BdComp.competitionIsActive)
            {
                if (!collisionDetected)
                    RammingDetection(); // Only log ramming in flight scenes. (Note: the 'competitionIsActive' check could be removed to allow checking for ramming outside of competitions.)
                if (collisionDetected && Planetarium.GetUniversalTime() - collisionTime > BDArmorySettings.RAM_LOGGING_COLLISION_UPDATE) // Note: if we collide with another plane within this time, then we'll lose the first collision.
                {
                    RammingLog();
        ...
        {
            GameEvents.onCollision.Remove(Collision);
        }
        ...
        public void RammingDetection()
        {
            ...
            //phase 1: check if this is a vessel and if there are vessels within the collision radius
            if (closestVessel != null)
            {
                ...
                if (possibleCollision && (int)vData.lastPossibleRammingTime == -1)
                {
                    ...
                    vData.hasReachedVessel = false;
                    vData.isHeadOn = false;
                    ...
                    if ((!vData.isRamming && !vData.otherVesselScoringData.isRamming) || (vData.isRamming && vData.otherVesselScoringData.isRamming))
                    {
                        ...
                        //check if vessels are both facing 20 degrees apart => isHeadOn = true
                        if (angleToTargetVessel < 20 && closestVesselAngleToTarget < 20) vData.isHeadOn = true;
                    }
                    else
                    {
            ...
            if (vData.rammedVessel == vessel && vData.lastHitTime > vData.lastPossibleRammingTime && (int)vData.lastPossibleRammingTime != -1)
                ResetBothRammingTimers();

            try
            {
                //if more than one vessel is within the collision logging radius => check for closest vessel
                if (vessel == vData.rammingVessel && vData.otherVesselScoringData.rammingVessel && vessel != vData.otherVesselScoringData.rammingVessel && (int)vData.lastPossibleRammingTime != -1 && !vData.otherVesselScoringData.hasReachedVessel)
                {
                    ...
                    else vData.lastPossibleRammingTime = -1;
                }
            }
            catch
            {
                Debug.Log("DEBUG vessel " + vessel.vesselName);
                Debug.Log("DEBUG vData.rammingVessel " + vData.rammingVessel);
                Debug.Log("DEBUG otherVessel " + vData.otherVesselScoringData);
                Debug.Log("DEBUG otherVessel rammingVessel " + vData.otherVesselScoringData.rammingVessel);
                Debug.Log("DEBUG otherVessel rammedVessel " + vData.otherVesselScoringData.rammedVessel);
                Debug.Log("DEBUG otherOtherVessel " + vData.otherVesselScoringData.otherVesselScoringData);
                Debug.Log("DEBUG otherOtherVessel last possible ramming time" + vData.otherVesselScoringData.otherVesselScoringData.lastPossibleRammingTime);
                throw;
            }


            //if this vessel is not the other ramming vessel or the other rammed vessel and the other vessel has already collided => reinitialize vessel
            if ((int)vData.lastPossibleRammingTime != -1 && (vData.otherVesselScoringData.rammingVessel && vData.otherVesselScoringData.rammedVessel) && (vessel != vData.otherVesselScoringData.rammedVessel || vessel != vData.otherVesselScoringData.rammingVessel) && vData.otherVesselScoringData.hasReachedVessel)
        ...
        {
            Debug.Log("DEBUG " + vessel.vesselName + " logging collision");
            string vesselName = vessel.GetName();
            ScoringData vData;
            if (BdComp.Scores.ContainsKey(vesselName))
                vData = BdComp.Scores[vesselName];
            ...
            //if vessel has been destroyed before ramming timer
            // if (vData.hasReachedVessel)
            // {
            ...
            {
                Debug.Log("DEBUG " + vessel.vesselName + " rammed " + vData.rammedVessel.vesselName + " and was destroyed");
                //log this vessels score
                LogRammingVesselScore(vessel, vData.rammedVessel, true, true, true);

                //add this vessels name to scoring data
                LogRammedVesselScore(vData.rammedVessel);

                //reset both timers
                ResetBothRammingTimers();
            }

            //check if the ramming vessel has been destroyed
            if ((int)vData.lastPossibleRammingTime != -1 && vessel == vData.rammedVessel && (vData.rammingVessel.FindPartModuleImplementing<MissileFire>() == null || vData.rammingVessel == null))
            {
                Debug.Log("DEBUG " + vessel.vesselName + " was rammed by " + vData.rammingVessel.vesselName + " and destroyed");
                //log ramming score as other vessel
                LogRammingVesselScore(vData.rammingVessel, vessel, false, true, true);

                //log score
                LogRammedVesselScore(vessel);
            ...
            //check if both vessels were destroyed
            if ((int)vData.lastPossibleRammingTime != -1 && vessel == vData.rammingVessel && vData.rammingVessel.FindPartModuleImplementing<MissileFire>() == null && vData.rammedVessel.FindPartModuleImplementing<MissileFire>() == null)
            {
                Debug.Log("DEBUG " + vData.rammingVessel.vesselName + " and " + vData.rammedVessel.vesselName + " rammed and destroyed each other");
                //Log ramming vessel score
                LogRammingVesselScore(vessel, vData.rammedVessel, true, false, true);
                LogRammedVesselScore(vData.rammedVessel);
                //log rammed vessel score as rammed vessel
                LogRammingVesselScore(vData.rammedVessel, vessel, true, false, true);
                LogRammedVesselScore(vessel);
                //set competition status
                BdComp.competitionStatus = vesselName + " AND " + vData.rammedVessel.GetName() + " COLLIDED AND LOST: " + Math.Abs(vData.closestVesselPartCountBeforeRam - vessel.parts.Count) + ", " + Math.Abs(vData.closestVesselPartCountBeforeRam - vData.rammedVessel.parts.Count) + " PARTS ";
                //reset both ramming timers
                ResetBothRammingTimers();
            ...
            if ((int)vData.lastPossibleRammingTime != -1)
            {
                Debug.Log("DEBUG " + vessel.vesselName + ", checking for damaged parts");
                //if vessel is not in a head on collision with other vessel
                if (!vData.isHeadOn)
                {
                    Debug.Log("DEBUG not head on");
                    ...
                    {
                        //log ramming score as this vessel
                        LogRammedVesselScore(vData.rammingVessel);
                    ...
                    //this vessel rammed and lost parts are detected on other vessel
                        ...
                        LogRammingVesselScore(vessel, vData.rammedVessel, false, true, true);

                    //reset ramming timer
                    vData.lastPossibleRammingTime = -1;
                }

                else if (vData.isHeadOn && vData.rammingVessel == vessel && vData.otherVesselScoringData.rammingVessel == vessel)
                {
                    Debug.Log("DEBUG head on");
                    //check if both vessels lost parts
                    if (vessel.parts.Count < vData.partCountBeforeRam && vData.rammedVessel.parts.Count < vData.closestVesselPartCountBeforeRam)
                    {
                        //log vessel scores
                        LogRammingVesselScore(vessel, vData.rammedVessel, false, false, true);
                        LogRammedVesselScore(vData.rammedVessel);
                        //log other vessel score
                        LogRammingVesselScore(vData.rammedVessel, vessel, false, false, true);
                        LogRammedVesselScore(vessel);
                        //set competition status
                        BdComp.competitionStatus = vesselName + " And " + vData.rammedVessel.GetName() + " RAMMED AND LOST: " + Math.Abs(vData.closestVesselPartCountBeforeRam - vessel.parts.Count) + ", " + Math.Abs(vData.closestVesselPartCountBeforeRam - vData.rammedVessel.parts.Count) + " PARTS";
                        //reset both timers
                        ResetBothRammingTimers();

                    }
                    //check if other vessel didn't lose parts
                    else if (vessel.parts.Count < vData.partCountBeforeRam && vData.rammedVessel.parts.Count! < vData.closestVesselPartCountBeforeRam)
                    {
                        //log other vessel as ramming vessel
                        LogRammingVesselScore(vData.rammedVessel, vessel, true, true, true);
                        //log this vessel as rammed vessel
                        LogRammedVesselScore(vessel);
                        //reset both timers
                        ResetBothRammingTimers();

                    }
                    //check if this vessel didn't lose parts
                    else if (vessel.parts.Count! < vData.partCountBeforeRam && vData.rammedVessel.parts.Count < vData.closestVesselPartCountBeforeRam)
                    {
                        //log this vessel as ramming vessel
                        LogRammingVesselScore(vessel, vData.rammedVessel, true, true, true);
                        //log rammed vessel score
                        LogRammedVesselScore(vData.rammedVessel);

                    }

                }
                //if no lost parts were detected => reset timers
                else
                {
                    Debug.Log("DEBUG no damage detected");
                    //reset timers
                    ResetBothRammingTimers();
                }
            }
        ...
        public void LogRammingVesselScore(Vessel v, Vessel t, bool isHeadShot, bool competitionStatus, bool log)
        {
            Debug.Log("DEBUG " + v.vesselName + " rammed " + t.vesselName);
            //initialize vData
            ScoringData vData = null;
            vData = BdComp.Scores[v.GetName()];

            //initialize part count
            var partCount = Math.Abs(vData.closestVesselPartCountBeforeRam - vData.rammedVessel.parts.Count);
            if (isHeadShot) partCount = vData.closestVesselPartCountBeforeRam;

            //log scores
            if (competitionStatus) { BdComp.competitionStatus = t.GetDisplayName() + " RAMMED BY " + v.GetDisplayName() + " AND LOST: " + partCount + " PARTS "; }
            if (log) { Debug.Log(t.GetName() + " RAMMED BY " + v.GetName() + " AND LOST: " + partCount + " PARTS "); }
            vData.totalDamagedParts += partCount;
            var damagedParts = partCount;
            if (BdComp.whoRammedWho.ContainsKey(v.GetName() + ":" + t.GetName()))
                BdComp.whoRammedWho[v.GetName() + ":" + t.GetName()] += damagedParts;
            else
                BdComp.whoRammedWho.Add(v.GetName() + ":" + t.GetName(), damagedParts);
        }

        public void LogRammedVesselScore(Vessel v)
        {
            Debug.Log("DEBUG " + v.vesselName + " got rammed");
            //initialize vData
            ScoringData vData = null;
            vData = BdComp.Scores[vessel.GetName()];
            ...
            if (!vData.everyoneWhoRammedMe.Contains(v.GetName()))
            {
                vData.everyoneWhoRammedMe.Add(v.GetName());
            }

            vData.lastPersonWhoRammedMe = v.GetName();
        }

        void ResetBothRammingTimers()
        {
            //initialize vData
            ScoringData vData = null;
            vData = BdComp.Scores[vessel.GetName()];
