    ...
    public abstract class MissileBase : EngageableWeapon, IBDWeapon
    {
        ...
        protected void UpdateHeatTarget()
        {
            ...
            if (lockFailTimer >= 0)
            {
                ...
                // Update heat target
                heatTarget = BDATargetManager.GetHeatTarget(SourceVessel, vessel, lookRay, predictedHeatTarget, lockedSensorFOV / 2, heatThreshold, allAspect, (SourceVessel != null ? SourceVessel.gameObject?.GetComponent<MissileFire>() : null)); // Unity messes with fake nulls and breaks ?. operators sometimes.
                ...
                if (predictedHeatTarget.exists)
                {
                    ...
                    predictedHeatTarget.velocity = predictedHeatTarget.velocity + predictedHeatTarget.acceleration * Time.fixedDeltaTime * Time.fixedDeltaTime;
