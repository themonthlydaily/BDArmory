...
namespace BDArmory.FX
{
    public class ExplosionFx : MonoBehaviour
    {
        ...
        private bool IsInLineOfSight(Part part, Part explosivePart, out RaycastHit hit, out float distance, out List<Tuple<float, float, float>> intermediateParts)
        {
			...
			var thickness = partHit.Modules.GetModule<HitpointTracker>();
                        float partArmour = thickness.Armor;
                        if (float.IsNaN(partArmour)) partArmour = 1;
                        partArmour *= thickness.Density * thickness.Strength; //placeholder
