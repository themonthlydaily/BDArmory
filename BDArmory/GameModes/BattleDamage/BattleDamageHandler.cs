        ...
        {      
            ...
            if (BDArmorySettings.BD_FIRES_ENABLED)
            {
                if (part.isBattery() && part.GetDamagePercentage() < 0.95f)
                {
                    ...
                    if (alreadyburning == null)
                    {
                        ...
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Battery Dice Roll: " + Diceroll);
            ...
            if (Armor != null)
            {
                if (Armor.HullTypeNum == 1) //wooden parts can potentially catch fire
                {
                    ...
                    {
                        double Diceroll = UnityEngine.Random.Range(0, 100);
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Wood part Dice Roll: " + Diceroll);
                        if (Diceroll <= BDArmorySettings.BD_DAMAGE_CHANCE)
                        {
            ...
            if (BDArmorySettings.BD_AMMOBINS && part.GetDamagePercentage() < 0.95f) //explosions have penetration of 0.5, should stop explosions phasing though parts from detonating ammo
            {
                ...
                if (ammo != null)
                {
                    ...
                    if (penetrationFactor > 1.2)
                    {
                        ...
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Ammo TAC DiceRoll: " + Diceroll + "; needs: " + damageChance);
            ...
            if (BDArmorySettings.BD_PROPULSION)
            {
                ...
                if (part.isEngine() && part.GetDamagePercentage() < 0.95f) //first hit's free
                {
                    foreach (var engine in part.GetComponentsInChildren<ModuleEngines>())
                    {
                        if (engine.thrustPercentage > BDArmorySettings.BD_PROP_FLOOR) //engines take thrust damage per hit
                        {
                            ...
                            if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: engine thrust: " + engine.thrustPercentage);
                ...
                if (BDArmorySettings.BD_INTAKES) //intake damage
                {
                    ...
                    if (intake != null)
                    {
                        ...
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Intake damage: Orig Area: " + tracker.origIntakeArea + "; Current Area: " + intake.area + "; Intake Speed: " + intake.intakeSpeed + "; intake damage: " + (1 - ((((tracker.oldDamagePercent - part.GetDamagePercentage())) * HEBonus) / BDArmorySettings.BD_PROP_DAM_RATE)));
                    }
                }
                if (BDArmorySettings.BD_GIMBALS) //engine gimbal damage
                {
                    ...
                    if (gimbal != null)
                    {
                        ...
                        double Diceroll = UnityEngine.Random.Range(0, 100);
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Gimbal DiceRoll: " + Diceroll);
            ...
            if (BDArmorySettings.BD_AEROPARTS && firsthit)
            {
                ...
                if (part.GetComponent<ModuleLiftingSurface>() != null)
                {
                    ...
                    if (wing.deflectionLiftCoeff > ((part.mass * 5) + liftDam)) //stock mass/lift ratio is 10; 0.2t wing has 2.0 lift; clamp lift lost at half
                    {
                        ...
                        if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: " + part.name + "hit by " + caliber + " round, penFactor " + penetrationFactor + "; took lift damage: " + liftDam + ", current lift: " + wing.deflectionLiftCoeff);
                    }
                }
            ...
            {
                double Diceroll = UnityEngine.Random.Range(0, 100);
                if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Subsystem DiceRoll: " + Diceroll + "; needs: " + damageChance);
                if (Diceroll <= (damageChance) && part.GetDamagePercentage() < 0.95f)
                {
                    ...
                    if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: " + part.name + "took subsystem damage");
            ...
            if (BDArmorySettings.BD_COCKPITS && penetrationFactor > 1.2f && part.GetDamagePercentage() < 0.9f && firsthit) //lets have this be triggered by penetrative damage, not blast splash
            {
                if (part.GetComponent<ModuleCommand>() != null)
                {
                    ...
                    if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Command DiceRoll: " + ControlDiceRoll);
                    if (ControlDiceRoll <= (BDArmorySettings.BD_DAMAGE_CHANCE * 2))
                    {
                        using (List<Part>.Enumerator craftPart = part.vessel.parts.GetEnumerator())
                        {
                            ...
                            if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: " + part.name + "took command damage");
                        }
                    }
                }
            }
            if (BDArmorySettings.BD_PILOT_KILLS)
            {
                ...
                if (canKill && part.protoModuleCrew.Count > 0 && penetrationFactor > 1.5f && part.GetDamagePercentage() < 0.95f && firsthit)
                {
                    ...
                    if (BDArmorySettings.DRAW_DAMAGE_LABELS) Debug.Log("[BDArmory.BattleDamageHandler]: Pilot TAC: " + PilotTAC + "; dice roll: " + killchance);
