    ...
    class BDAMutator : PartModule
    {
        ...
        public void EnableMutator(string name = "def") //FIXME - when using apply on timer and !apply global, this NREs
        {
            ...
            if (name == "def") //mutator not specified, randomly choose from selected mutators
            {
                ...
                if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDAMutator] random mutator list built: " + name + " on " + part.vessel.GetName());
            }
            mutatorName = name;
            ...
            for (int r = 0; r < BDArmorySettings.MUTATOR_APPLY_NUM; r++)
            {
                ...
                if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDAMutator] beginning mutator initialization of " + name + " on " + part.vessel.GetName());
        ...
        public void DisableMutator()
        {
            ...
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDAMutator]: Disabling " + mutatorInfo.name + "Mutator on " + part.vessel.vesselName);
            using (var weapon = VesselModuleRegistry.GetModules<ModuleWeapon>(vessel).GetEnumerator())
                while (weapon.MoveNext())
                {
                    if (weapon.Current == null) continue;
        ...
        void Detonate()
        {
            ...
            if (BDArmorySettings.DEBUG_LABELS) Debug.Log("[BDAMutator] triggering vengeance nuke");
