            ...
            if (missile is MissileBase)
            {
                ...
                if (guardMode)
                {
                    ...
                    if (ml.warheadType == MissileBase.WarheadTypes.EMP || ml.warheadType == MissileBase.WarheadTypes.Nuke)
                    {
                        ...
                        if (ml.StandOffDistance > 0 && (transform.position + (timeToImpact * (Vector3)vessel.Velocity())).FurtherFromThan(currentTarget.position + currentTarget.velocity, ml.StandOffDistance)) //if predicted craft position will be within blast radius when missile arrives, break off
                        {
